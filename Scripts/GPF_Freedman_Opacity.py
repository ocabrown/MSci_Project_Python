...
import numpy as np
import Freedman_Opacities_Function as FOF
...
class Freedman_Opacity_Giant_Planet:
    
    
    
    def __init__(self,mc,me,rp,pd,kd,Td,lp,met,acc_k,n,grid_given=False,m_grid=None,p_ana_c=None):
        ...
        Mp = core_mass_max + self.env_mass_max
        ...
        Z = met                         # Metallicity [log([M/H]/[M/H]_solar)]
        ...
        self._const = [core_mass_max, lum, Z, R]
    ...
    def Planet_Formation(self):
        ...
        l = self._const[1]
        Z = self._const[2]              # Metallicity [log([M/H]/[M/H]_solar)]
        R = self._const[3]              # Effective gas constant [erg/mol/K]
        ...
        for i in range(self._lo+1, self._hi+1):
            ...
            k[-(i+1)] = FOF.Freedman_Opacity(Z, P[-(i+1)], T[-(i+1)])
            
            A_adia = T[-i] * ((P[-(i+1)]/P[-i])**((gamma-1)/gamma))
            ...
            if abs(Grad_Rad) >= abs(Grad_Adia):             # Convective
                ...
                def g_func(A):
                    ...
                    F = FOF.Freedman_Opacity(Z, P[-(i+1)], A)
            ...
            """
                ...
                while (e_k > self._acc_k) or (e_T > self._acc_k):
                    ...
                    TL = (TR**4 - 3*k_avr*l*(m[-(i+1)]-m[-i])/(16*a*c*(np.pi**2)*r[-(i+1)]**4))**(1/4)
                    kL = FOF.Freedman_Opacity(Z, P[-(i+1)], TL)
